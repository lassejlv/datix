SELECT e.tracking_mode,e.tracking_settings,e.domain,e.allow_localhost,s.credit_budget::text,
  ...
  (SELECT coalesce(jsonb_agg(subscription),'[]'::jsonb)
    ...
    WHERE c.owner_id=s.owner_id AND c.deleted=false AND c.provider='polar' AND c.organization_id=$3 AND c.updated_at>now()-interval '5 minutes') AS subscriptions,
  (SELECT coalesce(jsonb_agg(jsonb_build_object('site',u.site_id,'start',u.period_start,'units',(u.events*100)::bigint)),'[]'::jsonb)
    FROM billing_organization_usage u WHERE u.owner_id=s.owner_id AND u.organization_id=$3 AND u.period_start>=now()-interval '32 days') AS usage
