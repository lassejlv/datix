...
const dir = 'web/artifacts/rust-production';
...
try {
  ...
  if (mode === 'cleanup') {
    ...
    if (mode === 'prepare' && !fixture) {
      const email = `rust-cutover-${crypto.randomUUID()}@example.com`,
        password = `Qa!${crypto.randomUUID()}`;
      const signup = await post('/auth/sign-up/email', {
        name: 'Rust cutover QA',
        email,
        password,
      });
      ...
      const created = await post('/sites', {
        name: 'Rust cutover QA',
        domain: 'rust-cutover.example.com',
      });
    ...
    const path = mode === 'prepare' ? '/before-rust' : '/after-rust';
    server = Bun.serve({
      hostname: '127.0.0.1',
      port: 0,
      fetch: () =>
        new Response(
          `<!doctype html><html><head><script defer src="${base}/tracker.js" data-site="${fixture!.site}"></script></head><body><h1>Cutover QA</h1><button data-analytics-ignore onclick="window.simpleAnalytics.track('rust_cutover')">Record event</button></body></html>`,
    ...
    await expect(
      page.getByRole('heading', { name: 'rust-cutover.example.com', exact: true }),
    ).toBeVisible();
