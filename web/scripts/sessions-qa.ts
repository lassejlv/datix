...
const client = new Client({ connectionString }),
  ...
  dir = `web/artifacts/${anonymous ? 'cookieless-activity' : localMode ? 'local-visitors' : 'sessions'}-${prod ? 'production' : 'local'}`;
...
try {
  ...
  if (!anonymous) {
    if (localMode) {
      // The UI has retired this option; preserve compatibility for existing environments.
      const changed = await context.request.patch(
        `${base}/api/sites/${site}/environments/${site}`,
        {
          headers: { origin: base },
          data: { trackingMode: 'local' },
        },
      );
    ...
    pass(
      localMode
        ? 'Existing local-storage environments retain consent-aware installation code'
