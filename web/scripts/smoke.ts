...
if (!['127.0.0.1', 'localhost'].includes(new URL(base).hostname))
  throw new Error(
    'This smoke runner creates disposable fixtures. Use it against the local Rust runtime only.',
  );
...
try {
  ...
  check('Rust/Axum health, frontend shell, tracker, and unauthenticated rejection');
  ...
  check('Compatible sign-up and cookie sessions in Rust against Neon');
  ...
  check('Actual Redis queue → Rust worker → Neon delivery and duplicate suppression');
  ...
  await writeFile(
    'web/artifacts/api-smoke.json',
    JSON.stringify(
      {
        verifiedAt: new Date().toISOString(),
        runtime: 'Local Rust/Axum web + Rust worker + Redis + isolated Neon test branch',
        checks,
      },
      null,
      2,
