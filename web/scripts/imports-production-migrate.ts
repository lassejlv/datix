...
try {
  const before = await client.query('SELECT version FROM analytics_schema_migrations ORDER BY version');
  console.log('Applied versions before:', before.rows.map(r => r.version));
  if (process.argv[2] === 'apply') {
    const child = Bun.spawn(['target/debug/analytics-db', 'upgrade'], {
      env: { ...process.env, DATABASE_URL: url.toString() }, stdout: 'pipe', stderr: 'pipe',
    });
    ...
    const grants = await client.query("SELECT has_table_privilege('analytics_runtime', 'analytics_imports', 'SELECT, INSERT, UPDATE, DELETE') AS imports, has_table_privilege('analytics_runtime', 'imported_daily_stats', 'SELECT, INSERT, UPDATE, DELETE') AS daily, has_table_privilege('analytics_runtime', 'imported_breakdowns', 'SELECT, INSERT, UPDATE, DELETE') AS breakdowns");
