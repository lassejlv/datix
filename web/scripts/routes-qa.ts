...
try {
  ...
  await page.getByLabel('Selected environment', { exact: true }).click();
  ...
  await mkdir('web/artifacts/routes', { recursive: true });
  await writeFile(
    `web/artifacts/routes/${prod ? 'production' : 'local'}.json`,
