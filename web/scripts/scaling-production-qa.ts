...
const dir = 'web/artifacts/scaling-production';
...
try {
  ...
  if (mode === 'cleanup') {
    ...
    if (mode === 'prepare' && !fixture) {
      const email = `scaling-cutover-${crypto.randomUUID()}@example.com`,
        password = `Qa!${crypto.randomUUID()}`;
      const signup = await post('/auth/sign-up/email', {
        name: 'Scaling cutover QA',
        email,
        password,
      });
      ...
      const created = await post('/sites', {
        name: 'Scaling cutover QA',
        domain: 'scaling-cutover.example.com',
      });
    ...
    const path = mode === 'prepare' ? '/before-scaling' : '/after-scaling';
    server = Bun.serve({
      hostname: '127.0.0.1',
      port: 0,
      fetch: () =>
        new Response(
          `<!doctype html><html><head><script defer src="${base}/tracker.js" data-site="${fixture!.site}"></script></head><body><h1>Cutover QA</h1><button data-analytics-ignore onclick="window.simpleAnalytics.track('scaling_cutover')">Record event</button></body></html>`,
    ...
    await expect(
      page.getByRole('heading', { name: 'scaling-cutover.example.com', exact: true }),
    ).toBeVisible();
