...
import { CheckoutConfetti } from './checkout-confetti';
...
export function AccountAccess({
  ...
  if (usage.data?.plan && !usage.error)
    return (
      <>
        <CheckoutConfetti active />
        {children(usage)}
      </>
    );

  const choosingPlan = Boolean(usage.data?.onboardingCompleted);
  const shell = choosingPlan ? 'max-w-[1080px]' : 'max-w-[680px]';

  return (
    <div className="min-h-dvh w-full bg-background text-foreground">
      <header
        className={`mx-auto flex w-full items-center justify-between gap-4 px-4 py-5 md:px-6 ${shell}`}
      >
        <Brand />
      ...
      <main className={`mx-auto w-full px-4 pt-5 pb-10 md:px-6 md:pt-8 ${shell}`}>
