(() => {
  ...
  function startSessions(cookieless = false) {
    ...
    // The site's consent manager owns persistence and must set this on each page.
    if (cookieless) {
      if (document.visibilityState === 'prerender')
        document.addEventListener('visibilitychange', () => setConsent(true), { once: true });
      else setConsent(true);
    } else if (window.analyticsBeerConsent === true) setConsent(true);
    else log('Visitor tracking is waiting for analytics consent from your consent banner.');
  }
})();
