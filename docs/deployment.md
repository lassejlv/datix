...
Analytics Beer runs in the [Analytics Beer Railway project](https://railway.com/project/ff5d734f-2a11-4fec-a74d-d2793515f557), production environment. Bun 1.4.2 is the runtime, build tool and package manager. `Dockerfile` pins that version and installs from `bun.lock` with `--frozen-lockfile`.

## Services

| Service | Command | Responsibility |
| --- | --- | --- |
| web | `bun run start` | HTTP API, TanStack Start SSR, and static assets |
| worker | `bun run start:worker` | Redis event consumption, billing delivery every minute, retention daily at 03:17 UTC |
| Redis | Redis 8.2 with persistent volume | BullMQ queues and shared atomic rate limits |

Web and worker stay running and use `/health/ready` as the deployment health check. The web service exposes port 3000. The worker has no public domain. Both health checks verify PostgreSQL and Redis; the worker also checks its consumers are running. SIGTERM drains active requests/jobs before closing connections.
...
The existing Neon production branch is retained: project `billowing-night-55335840`, branch `br-frosty-frost-b18zvthi`, database `analytics`, Frankfurt. No data copy or schema migration is needed for the hosting move. Each Bun process uses a PostgreSQL pool capped at 10 connections, with the restricted `analytics_runtime` role. Owner credentials in `.env.production` remain for explicit migrations only.

Both services require `APP_URL`, `DATABASE_URL`, `REDIS_URL`, `BETTER_AUTH_SECRET`, `VISITOR_HASH_SECRET`, `POLAR_ACCESS_TOKEN`, and `POLAR_WEBHOOK_SECRET`. Set `NODE_ENV=production` and `PORT=3000`. `REDIS_URL` references `${{Redis.REDIS_URL}}` over Railway private networking. The signing secrets are preserved from Cloudflare so existing cookies and visitor hashes remain valid. The web service also requires `CLOUDFLARE_ORIGIN_SECRET`, matching the `x-analytics-origin-key` request transform for `analytics.beer`. Bun checks this secret before accepting Cloudflare country metadata and removes the header before application handling. Railway already resolves `X-Real-IP` to the visitor; the direct Railway URL ignores unauthenticated country headers. Secrets are runtime variables; `.dockerignore` excludes environment files and local artifacts from the image.

## Deploying

The private source repository is [lassejlv/analytics-beer](https://github.com/lassejlv/analytics-beer). Both the `web` and `worker` services deploy from its `main` branch. Pushes to `main` trigger both services; their separate Railway start commands select the HTTP server or background worker. Redis continues to use its managed image and persistent volume.
...
railway up --project ff5d734f-2a11-4fec-a74d-d2793515f557 --environment 3f8cf4d9-b6d5-4532-8e1c-dc8ced8a1b76 --service 90c910b8-2705-4b6b-b08a-b7b19e2dd9de --detach
```

`bun run deploy` runs the checks and uploads both services with the pinned project/environment/service IDs. Wait for both deployments to report success, inspect runtime logs, then verify an authenticated account and an actual collector → Redis → worker → report round trip on the public domain. A build or health check alone is insufficient. Railway service settings hold the Dockerfile, start command, health check, restart policy and always-on configuration.
...
`src/cloudflare.ts`, `wrangler.jsonc`, and `bun run build:cloudflare` retain the former deployment path. The prior Worker and queues are retained for rollback. Both Cloudflare event/dead-letter queues were empty at cutover; HTTP routes and the old cron triggers are disabled. Reattaching its custom domain would switch traffic back; only do so as a deliberate rollback. Do not run `deploy:cloudflare` during normal Railway releases. Earlier deployment details and product verification records are in [Cloudflare deployment history](cloudflare-deployment-history.md).

## Migration verification — 2026-09-07

The cutover is complete. Web deployment `1ece80ac-7ab6-4746-ab6f-26237eec23f0` and worker deployment `bb213aa0-9559-424c-923a-577bb8ea3659` are healthy. Live verification covered existing-session continuity, browser tracker ingestion and reporting, country/locale handling, forged-header rejection, independent event IDs across environments, new sign-in/sign-out, and Polar API/signing credentials. The queue had 17 completed jobs with no waiting, active, delayed or failed jobs at the check. Billing and retention schedulers were present; both old Cloudflare queues were empty. The disposable account and temporary secret-transfer files were removed.
