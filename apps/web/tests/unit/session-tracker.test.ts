...
function tracker({
  ...
  blocked?: boolean;
  /** Leave analyticsBeerConsent undefined, as on a site that never sets the flag. */
  unset?: boolean;
} = {}) {
  const requests: any[] = [],
    timers: Function[] = [],
...
test('session mode tracks without consent wiring, because the site gates the snippet itself', async () => {
  ...
  expect(t.requests[0]).toMatchObject({ type: 'pageview', session: { kind: 'pageview' } });
  expect(t.writes.every((c) => c.includes('SameSite=Lax') && c.includes('Secure'))).toBe(true);
  // Withdrawal still works for a site that wires its banner to the callback.
...
test('local mode also starts on its own and still never writes cookies', async () => {
  const t = tracker({ mode: 'local', unset: true });
  await settle();
  expect(t.requests).toHaveLength(1);
  expect(t.requests[0].session.storage).toBe('local');
  expect(t.writes).toHaveLength(0);
});

test('session mode stops before cookies and requests when the page opts out with analyticsBeerConsent=false', async () => {
  const t = tracker();
  await settle();
