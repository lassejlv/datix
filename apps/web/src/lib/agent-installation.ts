...
2. ${
    environment.trackingMode !== 'cookieless'
      ? `Consent is this website's responsibility: the tracker collects as soon as it loads. Gate it on the existing consent banner's analytics choice, either by loading the script only after consent or by setting window.analyticsBeerConsent = false before it loads. If there is no banner, add accept/reject choices and a way to withdraw consent. Apply its saved choice on every page, including before the deferred tracker loads:
...
Call this on initial consent restoration and whenever the choice changes. Rejection or withdrawal must pass false. The callback does not render a banner. Verify that no tracking identifiers or activity requests appear before consent, that tracking works after acceptance, and that withdrawal stops collection and removes tracking identifiers. Keep private sections excluded with data-analytics-ignore; never add personal data to analytics labels.`
