...
try {
  ...
  await page.screenshot({ path: 'artifacts/sign-in-dark.png', fullPage: true });
  ...
  await page.screenshot({ path: 'artifacts/installation-desktop.png', fullPage: true });
  ...
  // Map the reserved QA hostname onto the local Worker. This executes the real
  ...
  await tracking.route(`https://${domain}/**`, async (route) => {
    ...
    if (url.pathname === '/' || url.pathname === '/work') {
      await route.fulfill({
        ...
        body: `<html><head><title>QA website</title><script defer src="/tracker.js" data-site="${siteId}"></script></head><body><h1>QA tracker fixture</h1><button onclick="history.pushState({}, '', '/work')">Work</button><button onclick="window.simpleAnalytics.track('contact')">Contact</button></body></html>`,
      });
    } else {
  ...
  for (let day = 0; day < 30; day++) {
    ...
    const count = 4 + Math.round(day * 1.4) + (day % 5) * 3;
    for (let index = 0; index < count; index++)
      items.push({
        ...
        type: index % 12 === 0 ? 'event' : 'pageview',
        name: index % 12 === 0 ? 'contact' : '',
  ...
  for (let index = 0; index < items.length; index += 100)
    await ingest(database(client), items.slice(index, index + 100));
  ...
  const selectedFill = await firstMetric.evaluate((el) => getComputedStyle(el).backgroundColor);
  expect(selectedFill).not.toBe('rgba(0, 0, 0, 0)');
  await firstMetric.focus();
  await page.keyboard.press('Tab');
  await page.keyboard.press('Shift+Tab');
  expect(await firstMetric.evaluate((el) => getComputedStyle(el).outlineWidth)).toBe('2px');
  await page.getByRole('button', { name: 'Events', exact: false }).hover();
  expect(
    await page
      .getByRole('button', { name: 'Events', exact: false })
  ...
  const localServer = Bun.serve({
    ...
    fetch: () =>
      new Response(
        `<html><head><script defer src="${base}/tracker.js" data-site="${siteId}"></script></head><body><button onclick="window.simpleAnalytics.track('localhost_test')">Local event</button></body></html>`,
  ...
  try {
    ...
    await page.screenshot({ path: 'artifacts/localhost-settings-mobile.png', fullPage: true });
    await page.getByRole('button', { name: 'Toggle navigation' }).click();
    await page.getByRole('link', { name: 'Installation', exact: true }).click();
    await expect(setting).toBeChecked();
    await page.setViewportSize({ width: 1440, height: 1024 });
    await page.screenshot({ path: 'artifacts/localhost-installation-desktop.png', fullPage: true });
    await setting.click();
    await expect(setting).not.toBeChecked();
  ...
  await expect(page.getByRole('button', { name: 'Add your first website' })).toBeVisible();
  passed(
    'Neutral light/dark themes, visible focus, hover and selected states, mobile navigation, rename, pause/resume, and guarded deletion',
  );
  await page.getByRole('button', { name: 'Toggle navigation' }).click();
    ...
    JSON.stringify(
      {
        ...
        desktop: [1440, 1024],
        mobile: [390, 844],
        runtime: 'Chromium, local Workerd, remote Neon',
