...
try {
  ...
  for (const width of [1440, 390]) {
    ...
    for (const theme of ['light', 'dark'] as const) {
      ...
      await page.screenshot({ path: `artifacts/dither-${width}-${theme}.png`, fullPage: true });
      results.push({
        width,
        theme,
        ink: expected,
  ...
  await page.screenshot({ path: 'artifacts/chart-failure.png', fullPage: true });
  throw error;
} finally {
